...
class DQAgent:
    ...
    def get_random_state(self):
        ...
        return self.max_loss_memory.tree.data[randrange(0, self.max_loss_memory.tree.data_pointer)]['source']
    ...
    def add_experience(self, source, action, reward, dest, final):
        ...
        self.max_loss_memory.store({'source': source,
                                           'action': action,
                                           'reward': reward,
                                           'dest': dest,
                                           'final': final})
        ...
        if (self.max_loss_memory.tree.data_pointer % 100 == 0) and (self.max_loss_memory.tree.data_pointer < self.replay_memory_size) and (self.logger is not None):
            self.logger.log("Gathered %d samples of %d" %
                            (self.max_loss_memory.tree.data_pointer, self.replay_memory_size))
    ...
    def sample_batch(self):
        ...
        return self.max_loss_memory.sample(self.minibatch_size)
    ...
    def train(self):
        ...
        batch_idx, batch, ISWeights = self.sample_batch()

        # train and dqn and update the priority
        self.DQN.train(batch_idx, batch, ISWeights, self.DQN_target, self.max_loss_memory)
