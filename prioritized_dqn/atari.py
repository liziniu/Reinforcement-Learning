...
parser.add_argument('--replay-memory-size', type=int, default=1000,
                    help='number of samples stored in the replay memory')
parser.add_argument('--target-network-update-freq', type=int, default=100,
                    help='frequency (number of DQN updates) with which the '
                         'target DQN is updated')
parser.add_argument('--avg-val-computation-freq', type=int, default=100,
                    help='frequency (number of DQN updates) with which the '
...
parser.add_argument('--test-freq', type=int, default=500,
                    help='frequency (number of frames) with which to test the '
                         'agent\'s performance')
parser.add_argument('--validation-frames', type=int, default=1350,
...
network_input_shape = (4, 84, 84)  # Dimension ordering: 'th' (channels first)
...
if args.train:
    ...
    while episode < args.max_episodes:
        ...
        while t < args.max_episode_length:
            ...
            # Train the agent
            if t % args.update_freq == 0 and DQA.max_loss_memory.tree.data_pointer >= args.replay_start_size:
                DQA.train()
            ...
            if DQA.max_loss_memory.tree.data_pointer >= args.replay_start_size:
                DQA.update_epsilon()
